...
/**
 * Used to Generate everything that is a Item It uses the already existing Gernerators as help
 * ItemDataGenerator = itemGen WorldIteBuilder = worldGen
 */
public class GeneralGenerator {
    ...
    /**
     * creates the Itemdata for the Magic book with the itemGen then makes it into a Worlditem with
     * the worlGen# (this spawnes the Item in a random space on the map)
     */
    public void createMagicBook() {
        ...
        generalGenLogger.log(
                CustomLogLevel.INFO,
                item.getItemName()
                        + "of type: "
                        + item.getItemType()
                        + " was created and has been added to the Game");
    }
    ...
    public void dropItems(ItemData itemData, Point pos) {
        ...
        generalGenLogger.log(
                CustomLogLevel.INFO,
                "Item: " + itemData.getItemName() + " was dropped at" + pos.x,
                pos.y);
    }
        ...
        for (int i = 0; i < amount; i++) {
            ...
            generalGenLogger.log(
                    ...
                    item.getItemName()
                            + "of type: "
                            + item.getItemType()
                            + " was created and will be added to inventory");
        }
        return temp;
    }

    /**
     * creates the loot for stronger monster this Monster drops two different types of Loot Food and
     * Potion Potions are worth more, so they take 2 Item amounts
     *
     * @param amount
     * @return
     */
    ...
    /**
     * since the bagpack is a Special Item it isnt created by the ItemDataGenerator however in order
     * to drop it the worldGen will still be used
     */
    public void makeBagPack() {
        ...
        generalGenLogger.log(
                ...
                bag.getItemName()
                        + "of type: "
                        + bag.getItemType()
                        + " was created and will be added map");
    }

    /**
     * iOnUse method that food Items will be given since they don´t just vanish but also replenish
     * HP
     *
     * @param e
     * @param item
        ...
        e.getComponent(HealthComponent.class)
                .ifPresent(
                        component -> {
                            ...
                            healthcomp.setCurrentHealthpoints(
                                    healthcomp.getCurrentHealthpoints() + 2);
                            System.out.println(
                                    "Current health: "
                                            + healthcomp.getMaximalHealthpoints()
                                            + " / "
                                            + healthcomp.getCurrentHealthpoints());
                        });
    }
    /**
    ...
    public void useBook(Entity e, ItemData item) {
        ...
        e.getComponent(HealthComponent.class)
                .ifPresent(
                        component -> {
                            HealthComponent healthcomp = (HealthComponent) component;
                            healthcomp.setMaximalHealthpoints(
                                    healthcomp.getMaximalHealthpoints() + 5);
                            System.out.println(
                                    "Current health: "
                                            + healthcomp.getMaximalHealthpoints()
                                            + " / "
    ...
    /**
     * iOnUse method that Potion Items will be given since they don´t just vanish but also increase
     * Speed
     *
     * @param e
     * @param item
