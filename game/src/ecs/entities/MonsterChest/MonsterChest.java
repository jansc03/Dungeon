...
public class MonsterChest extends Entity {
    ...
    /**
     * called on death
     * creates ChestMonster at the Position of the Chest and with the Inventory
     * @param e
     */
    public void spawnMonster(Entity e){
        e.getComponent(PositionComponent.class).ifPresent(c -> {
               e.getComponent(InventoryComponent.class).ifPresent(i ->{
                   ChestMonster chestMonster = new ChestMonster(((InventoryComponent)i).getItems(),((PositionComponent)c).getPosition());
                   LOGGER.log(CustomLogLevel.INFO,this.getClass().getSimpleName()+" has Spawned :"+chestMonster.getClass().getSimpleName());
               });
            });
